...
public class ReservationEventConsumer {
    ...
    // 예매 완료/취소 - 슬롯 1개 증가
    @KafkaListener(topics = {"reservation.completed", "reservation.cancelled"}, groupId = "queue-service")
    @IdempotentConsumer("queue-service")
    public void handleReservationEvent(ConsumerRecord<String, String> record, Acknowledgment ack) throws JsonProcessingException {
        SlotsReleaseEvent event = null;
        try {
    ...
    // DLT 이벤트 수신
    @KafkaListener(topics = {"reservation.completed.DLT", "reservation.cancelled.DLT"}, groupId = "queue-service")
    @IdempotentConsumer("queue-service")
    public void handleReservationDltEvent(ConsumerRecord<String, String> record, Acknowledgment ack) {
        SlotsReleaseEvent event = null;
        try {
            Header traceHeader = record.headers().lastHeader("traceId");
            String traceId = traceHeader != null
            ...
            event = objectMapper.readValue(record.value(), SlotsReleaseEvent.class);
            log.info("[queue-service] dlt 수신, 슬롯 반환 수동 처리 필요: {}", event.matchId());
